  ...
  module V1
    module Items
      class SearchController < ApplicationController
        def index
          if params[:min_price]
            return render json: ErrorSerializer.bad_data, status: :bad_request if params[:min_price].to_f.negative?
          elsif params[:max_price]
            return render json: ErrorSerializer.bad_data, status: :bad_request if params[:max_price].to_f.negative?
          end
